...
Rectangle {
    ...
    // Line+area chart for a rolling series. `values` is oldest-first, so the
    // line reads left to right. `maxValue` fixes the vertical scale; 0
    // auto-scales to the window's own peak, floored by `minScale` so an idle
    // series does not magnify stray bytes.
    ...
    // Caption + Sparkline + value readout. Sparkline is local (above); the
    // per-cell `lineColor` is why colors still arrive as properties here.
    component Cell: Row {
        id: cell
